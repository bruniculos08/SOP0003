...
int main(int argc, char *argv[])
{
    ...
    { /* filho => consumidor */
        while (!sai)
        {
            // Obs.: a função sem_wait() retorna -1 em caso de erro, isto é, caso o semáforo não exista ou não esteja...
            // ... acessível;
            if (sem_wait(sem_full) == -1)
            { /* => down(&sem_full) + verificacao de erro */
                perror("sem_wait");
                break;
            }
            printf("Consumindo: %d\n", *ptr);
            if (sem_post(sem_empty) == -1)
            { /* => up(&sem_empty) + verificacao de erro */
                perror("sem_post");
                break;
            }
            if (*ptr == 0)
    ...
    else
    {
        while (!sai)
        {
            ...
            { /* => down(&sem_empty) + verificacao de erro */
                perror("sem_wait");
                break;
            }
            *ptr = produz();
            ...
            { /* => up(&sem_full) + verificacao de erro */
                perror("sem_post");
                break;
            }
            if (*ptr == 0)
